...
- **File serving, upload & editing** — devices can fetch and upload files; device-specific files take precedence over project-wide defaults (ETag caching supported). In the UI, browse files with a drill-down editor, preview images, and edit JSON as raw text or as a form — optionally driven by an uploaded schema you approve ([docs/concepts.md](docs/concepts.md#schema-driven-json-forms))
...
| [Core Concepts](docs/concepts.md) | Data storage, token model, device lifecycle, file editing and schema forms, firmware distribution, alarms, system health |
